...
class VelocityLoss(LossFunctionWrapper):
    """Applies a L2 penalty to velocity error between the model's output velocity state ``x`` and a user-fed
    label velocity ``y``:

    .. code-block:: python

        xp, xv = np.split(x, 2, axis=-1)  # remove position from the velocity state
        yp, yv = np.split(y, 2, axis=-1)
        loss = np.reduce_mean(np.abs(xv - yv))

    .. note::
        The velocity error does not include position, hence the use of ``np.split`` to extract velocity from the
        state array.
    ...
    def __init__(self, name: str = 'velocity', reduction=auto_reduction):
        super().__init__(_velocity_loss, name=name, reduction=reduction)
...
def _velocity_loss(y_true, y_pred):
